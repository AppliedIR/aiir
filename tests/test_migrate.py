...
def _make_old_case(case_dir, examiner="alice", findings=None, timeline=None,
                   todos=None, actions=None, approvals=None, audit=None):
    """Create old examiners/ directory structure for migration testing."""
    case_dir.mkdir(exist_ok=True)
    meta = {"case_id": "INC-2026-001", "name": "Test case", "status": "open",
            "mode": "collaborative", "team": ["alice", "bob"]}
...
class TestBasicMigration:
    ...
    def test_findings_re_id(self, tmp_path, monkeypatch, identity):
        ...
        _make_old_case(case_dir, examiner="alice", findings=[
            ...
            {"id": "F-002", "status": "DRAFT", "title": "Lateral movement",
    ...
    def test_timeline_re_id(self, tmp_path, monkeypatch, identity):
        case_dir = tmp_path / "case1"
        _make_old_case(case_dir, examiner="alice", timeline=[
            {"id": "T-001", "timestamp": "2026-01-10T08:00:00Z",
             "description": "Login event", "staged": "2026-01-10T12:00:00Z"},
        ])
        monkeypatch.setenv("AIIR_CASE_DIR", str(case_dir))
        monkeypatch.setenv("AIIR_EXAMINER", "alice")
        args = Namespace(case=None, examiner="alice", import_all=False)
    ...
    def test_todos_re_id(self, tmp_path, monkeypatch, identity):
        case_dir = tmp_path / "case1"
        _make_old_case(case_dir, examiner="alice", todos=[
            {"todo_id": "TODO-001", "description": "Check logs",
    ...
    def test_todo_related_findings_updated(self, tmp_path, monkeypatch, identity):
        ...
        _make_old_case(case_dir, examiner="alice",
                       ...
                       todos=[
                           {"todo_id": "TODO-001", "description": "Verify",
                            "related_findings": ["F-001"]},
                       ])
        monkeypatch.setenv("AIIR_CASE_DIR", str(case_dir))
        monkeypatch.setenv("AIIR_EXAMINER", "alice")
    ...
    def test_actions_migrated(self, tmp_path, monkeypatch, identity):
        case_dir = tmp_path / "case1"
        _make_old_case(case_dir, examiner="alice", actions=[
            {"action": "exec", "tool": "log2timeline", "ts": "2026-01-10T12:00:00Z"},
        ])
        monkeypatch.setenv("AIIR_CASE_DIR", str(case_dir))
        monkeypatch.setenv("AIIR_EXAMINER", "alice")
        args = Namespace(case=None, examiner="alice", import_all=False)
        ...
        entries = [json.loads(line) for line in actions_file.read_text().strip().split("\n")]
...
class TestApprovalCrossReferences:
    ...
    def test_approval_item_id_updated(self, tmp_path, monkeypatch, identity):
        ...
        _make_old_case(case_dir, examiner="alice",
                       ...
                       approvals=[
                           {"item_id": "F-001", "action": "APPROVED",
                            "examiner": "alice", "ts": "2026-01-10T13:00:00Z"},
                       ])
        monkeypatch.setenv("AIIR_CASE_DIR", str(case_dir))
        monkeypatch.setenv("AIIR_EXAMINER", "alice")
        args = Namespace(case=None, examiner="alice", import_all=False)
        ...
        entries = [json.loads(line) for line in approvals_file.read_text().strip().split("\n")]
        assert len(entries) == 1
    ...
    def test_approval_scoped_id_updated(self, tmp_path, monkeypatch, identity):
        ...
        _make_old_case(case_dir, examiner="alice",
                       ...
                       approvals=[
                           {"item_id": "alice/F-001", "action": "APPROVED",
                            "examiner": "alice", "ts": "2026-01-10T13:00:00Z"},
                       ])
        monkeypatch.setenv("AIIR_CASE_DIR", str(case_dir))
        monkeypatch.setenv("AIIR_EXAMINER", "alice")
...
class TestImportAll:
    ...
    def test_merge_two_examiners(self, tmp_path, monkeypatch, identity):
        case_dir = tmp_path / "case1"
        _make_old_case(case_dir, examiner="alice", findings=[
            {"id": "F-001", "status": "DRAFT", "title": "Alice finding",
             "staged": "2026-01-10T12:00:00Z"},
        ])
        # Add bob's directory
        bob_dir = case_dir / "examiners" / "bob"
        bob_dir.mkdir(parents=True)
        (bob_dir / "findings.json").write_text(json.dumps([
            {"id": "F-001", "status": "DRAFT", "title": "Bob finding",
             "staged": "2026-01-11T12:00:00Z"},
        ]))
        (bob_dir / "timeline.json").write_text(json.dumps([
            {"id": "T-001", "timestamp": "2026-01-11T09:00:00Z",
             "description": "Bob event", "staged": "2026-01-11T12:00:00Z"},
        ]))
    ...
    def test_import_all_primary_first(self, tmp_path, monkeypatch, identity):
        ...
        case_dir = tmp_path / "case1"
        _make_old_case(case_dir, examiner="alice", findings=[
            {"id": "F-001", "status": "DRAFT", "title": "Alice first",
             "staged": "2026-01-10T12:00:00Z"},
        ])
        bob_dir = case_dir / "examiners" / "bob"
        bob_dir.mkdir(parents=True)
        (bob_dir / "findings.json").write_text(json.dumps([
            {"id": "F-001", "status": "DRAFT", "title": "Bob first",
             "staged": "2026-01-11T12:00:00Z"},
        ]))
    ...
    def test_import_all_merges_approvals(self, tmp_path, monkeypatch, identity):
        case_dir = tmp_path / "case1"
        _make_old_case(case_dir, examiner="alice",
                       findings=[{"id": "F-001", "status": "APPROVED", "title": "A",
                                  "staged": "2026-01-10T12:00:00Z"}],
                       approvals=[{"item_id": "F-001", "action": "APPROVED",
                                   "examiner": "alice"}])
        bob_dir = case_dir / "examiners" / "bob"
        bob_dir.mkdir(parents=True)
        (bob_dir / "findings.json").write_text(json.dumps([
            {"id": "F-001", "status": "APPROVED", "title": "B",
             "staged": "2026-01-11T12:00:00Z"},
        ]))
        with open(bob_dir / "approvals.jsonl", "w") as f:
            f.write(json.dumps({"item_id": "F-001", "action": "APPROVED",
                                "examiner": "bob"}) + "\n")
...
class TestBackup:
    ...
    def test_existing_backup_not_overwritten(self, tmp_path, monkeypatch, identity, capsys):
...
class TestCorruptJsonGraceful:
    ...
    def test_corrupt_approvals_jsonl_line_skipped(self, tmp_path, monkeypatch, identity):
        case_dir = tmp_path / "case1"
        _make_old_case(case_dir, examiner="alice",
                       findings=[{"id": "F-001", "status": "APPROVED", "title": "A",
                                  "staged": "2026-01-10T12:00:00Z"}])
...
class TestExistingFlatFilesRefusesOverwrite:
    ...
    def test_existing_timeline_with_data(self, tmp_path, monkeypatch, identity):
        ...
        (case_dir / "timeline.json").write_text(json.dumps([
            {"id": "T-alice-001", "timestamp": "2026-01-10T08:00:00Z"}
        ]))
    ...
    def test_existing_todos_with_data(self, tmp_path, monkeypatch, identity):
        ...
        (case_dir / "todos.json").write_text(json.dumps([
            {"todo_id": "TODO-alice-001", "description": "Existing"}
        ]))
...
class TestExaminerNotFound:
    ...
    def test_examiner_not_found_shows_available(self, tmp_path, monkeypatch, identity, capsys):
        case_dir = tmp_path / "case1"
        _make_old_case(case_dir, examiner="alice", findings=[])
...
class TestMissingOptionalFiles:
    ...
    def test_findings_only(self, tmp_path, monkeypatch, identity):
        ...
        case_dir = tmp_path / "case1"
        _make_old_case(case_dir, examiner="alice", findings=[
            {"id": "F-001", "status": "DRAFT", "title": "Only finding",
             "staged": "2026-01-10T12:00:00Z"},
        ])
        monkeypatch.setenv("AIIR_CASE_DIR", str(case_dir))
        monkeypatch.setenv("AIIR_EXAMINER", "alice")
...
class TestEvidenceCopy:
    ...
    def test_evidence_access_appended(self, tmp_path, monkeypatch, identity):
        ...
        with open(exam_dir / "evidence_access.jsonl", "w") as f:
            f.write(json.dumps({"action": "mount", "ts": "2026-01-10T12:00:00Z"}) + "\n")
        ...
        entries = [json.loads(line)
                   for line in access_file.read_text().strip().split("\n")]
        assert len(entries) == 1
